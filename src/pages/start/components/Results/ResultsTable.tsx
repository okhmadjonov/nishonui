...
const getBaho = (shots: number[], lastSum: number) => {
  const { last } = splitShots(shots);
  const hits = last.filter((s) => s > 0).length;
  const totalShots = shots.length;
  const isPM = totalShots <= 8 || totalShots === 9 ? true : totalShots < 10;
  const isAutomat = totalShots >= 10;

  if (isPM) {
    if ((lastSum >= 41 && lastSum <= 50)) return "аъло";
    if ((lastSum >= 31 && lastSum <= 40)) return "яхши";
    if ((lastSum >= 20 && lastSum <= 30)) return "қониқарли";
    return "қониқарсиз";
  }
  if (isAutomat) {
    if ((lastSum >= 56 && lastSum <= 70)) return "аъло";
    if ((lastSum >= 36 && lastSum <= 55)) return "яхши";
    if ((lastSum >= 25 && lastSum <= 35)) return "қониқарли";
    return "қониқарсиз";
  }
  return "қониқарсиз";
};
...
export default function ResultsTable({
  ...
  const handleDownloadAllPDF = async () => {
    ...
    doc.text("MA'NAVIY-MA'RIFIY ISHLAR VA KADRLAR BILAN TA'MINЛАSH DEPARTAMENTI", pageWidth / 2, y + imgHeight + 34, { align: "center" });
    ...
    const head = [["Id", "Rasm", "F.I.O", "Lavozim", "Unvon", "Umumiy", "Baho", "Test", "Last"]];
    ...
    const wBaho = 60;
    const wTest = 90;
    const usedWidth = wId + wPhoto + wName + wLavozim + wUnvon + wScore + wBaho + wTest;
    const wLast = tableMaxWidth - usedWidth - 5;

    const rowsWithMeta = await Promise.all(results.map(async (r) => {
      const { test, last } = splitShots(r.shots || []);
      const baho = getBaho(r.shots || [], r.score || 0);
    ...
    autoTable(doc, {
      startY,
      head,
      body: rowsWithMeta.map(r => r.pdfRow),
      ...
      didDrawCell: (data: any) => {
        if (data.section === "body" && data.column.index === 1) {
          ...
          const rowMeta = rowsWithMeta[rowIdx];
          const dataUrl = rowMeta?.imageData;
          if (dataUrl) {
  ...
  const columns: ColumnsType<ResultRow> = [
    {
      ...
      render: (v: string | number) => <div className={styles.wrapCellInner}>{String(v).slice(0, 3)}</div>,
    },
    {
      title: "Rasm",
      dataIndex: "photoUrl",
    ...
    },
    {
      ...
      render: (_: number[], record) => (
        <div className={styles.wrapCellInner}>{getBaho(record.shots || [], record.score || 0)}</div>
      ),
    },
    {
      title: "Test (1-3)",
